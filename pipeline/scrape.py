...
def scrape_group_incremental(context, slug: str, group_name: str) -> None:
    ...
    try:
        ...
        print(f"{group_name} ({slug}): {len(listed)} past events listed, {len(new_nodes)} new", flush=True)
        ...
        for node in new_nodes:
            ...
            try:
                ...
                new_events.append({"event_url": url, "event_id": event_id, "error": str(e)})
            time.sleep(1)
...
def main():
    ...
    with sync_playwright() as p:
        context = p.chromium.launch_persistent_context(str(config.PROFILE_DIR), headless=True)
        for group in groups:
            scrape_group_incremental(context, slug_from_url(group["url"]), group["name"])
        context.close()
