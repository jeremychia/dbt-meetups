...
ENRICHED_DIR = os.environ.get(
    "DASHBOARD_ENRICHED_DIR", os.path.join(ANALYSIS_DIR, "enriched")
)
SPEAKER_ID_FILE = os.environ.get(
    "DASHBOARD_SPEAKER_FILE",
    os.path.join(ANALYSIS_DIR, "pipeline", "speaker-identities.json"),
)
OUTPUT_FILE = os.environ.get(
    "DASHBOARD_OUTPUT_FILE", os.path.join(DASHBOARD_DIR, "dashboard_data.json")
)
...
def chapter_slug(fname):
    return (
        fname.replace("-dbt-meetup-group.json", "")
...
def build_output(enriched_dir, speaker_id_file, today=None):
    ...
    topic_sample_talks = defaultdict(
        list
    )  # category -> [{title, speaker, chapter, date, attendees}]
    ...
    for path in sorted(glob.glob(f"{enriched_dir}/*.json")):
        ...
        region_info = (
            region_for(geo["country"])
            if geo and geo.get("country")
            else {"sub_region": None, "region_group": None}
        )
        with open(path) as f:
            data = json.load(f)
        ...
        for event in events:
            ...
            if d:
                ...
                try:
                    weekday_counter[
                        DAY_NAMES[datetime.date.fromisoformat(d).weekday()]
                    ] += 1
            ...
            for talk in event.get("talks") or []:
                ...
                if d and in_trailing_year(d, today_iso, trailing_start):
                    ...
                    ch_trailing_talks += 1
                if d and in_prior_trailing_year(
                    d, prior_trailing_start, prior_trailing_end
                ):
                ...
                if primary_cat:
                    ...
                    if region_info["region_group"]:
                        region_category_counter[region_info["region_group"]][
                            primary_cat
                        ] += 1
        ...
        if geo is None:
            print(
                f"WARNING: {slug} missing from chapter_geo.py — add it (skipping map placement)"
            )
        ...
        chapters[slug] = {
            ...
            "avg_attendees": (
                round(sum(ch_attendees) / len(ch_attendees), 1)
                if ch_attendees
                else None
            ),
            ...
            "events_detail": sorted(
                ch_events_detail, key=lambda e: e["date"] or "", reverse=True
            ),
            "is_active": is_active,
            ...
            "trailing_12mo_avg_attendees": (
                round(sum(ch_trailing_attendees) / len(ch_trailing_attendees), 1)
                if ch_trailing_attendees
                else None
            ),
        }
    ...
    events_yoy_pct = (
        round(
            100
            * (trailing_events_total - prior_trailing_events_total)
            / prior_trailing_events_total,
            1,
        )
        if prior_trailing_events_total
        else None
    )
    ...
    output = {
        ...
        "summary": {
            ...
            "date_range": [
                min(monthly_events.keys()) if monthly_events else None,
                max(monthly_events.keys()) if monthly_events else None,
            ],
            ...
            "trailing_12mo_avg_talks_per_event": (
                round(trailing_talks_total / trailing_events_total, 2)
                if trailing_events_total
                else None
            ),
        ...
        "region_category_distribution": {
            region: counter.most_common()
            for region, counter in region_category_counter.items()
        },
        "category_topics": {
        ...
        "region_yearly_events": {
            region: dict(sorted(years.items()))
            for region, years in region_yearly_events.items()
        },
        "weekday_event_counts": {day: weekday_counter.get(day, 0) for day in DAY_NAMES},
        "day_of_month_event_counts": {
            str(d): day_of_month_counter.get(d, 0) for d in range(1, 32)
        },
        "start_hour_event_counts": {
            str(h): start_hour_counter.get(h, 0) for h in range(24)
        },
