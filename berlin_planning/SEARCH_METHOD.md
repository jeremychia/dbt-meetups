...
- **Dataset version:** 3 (shared schema version 1)
...
- **Sister dataset:** `../baltics/lithuania_dbt_companies.json`. Both files follow one shared schema (§3). They are named `<region>_dbt_companies.json`, and a validator checks that their keys are identical (Appendix A).
...
- **`priority_tier`**, set by judgement:
  - **1:** strong, recent, in-region.
  - **2:** good lead, needs verification, or gave a relevant talk at another meetup.
  - **3:** older, peripheral or remote.
...
`berlin_planning/berlin_dbt_companies.json` and `baltics/lithuania_dbt_companies.json` have exactly these keys, in this order. A value can be `null` when it's unknown, but a key is never missing.
...
- **`metadata.counts`** uses the same keys in both files (`standard_counts()` in Appendix A).
- **Definitions** for every value are in `metadata.field_definitions`. That object is identical in both files.
- **Changing the schema:** a schema change goes into both files at once. Bump `schema_version`, update the key lists in Appendix A, and update this section in both SEARCH_METHOD.md files.

---

## 4. How to update the dataset

1. **Back up, then merge. Don't overwrite.**
   - Copy the current file to `berlin_dbt_companies.v<N>.json`.
...
- Back up the old file as berlin_dbt_companies.v<N>.json, merge following §4, run the Appendix A
...
Run it from the repo root. It checks both regional files.
...
PERSON = ["id","name","title","city","based_in_region","level","linkedin_urls","has_linkedin",
          "linkedin_confidence","meetup_fit","mentions_dbt","speaker_evidence","attendee_signal","evidence",
...
b = check("berlin_planning/berlin_dbt_companies.json")
lt = check("baltics/lithuania_dbt_companies.json")
assert b["metadata"]["field_definitions"] == lt["metadata"]["field_definitions"], "field_definitions differ"
assert list(b["metadata"]["counts"]) == list(lt["metadata"]["counts"]), "counts keys differ"
assert b["metadata"]["schema_version"] == lt["metadata"]["schema_version"]
print("ok")
```
